...
export function renderLanding(origin, nonce) {
  ...
  const data = escapeScriptJson(getConverterData());
  const featuredMarkup = featured
    .map(
      entry =>
        `<li><span class="dot dot-${entry.category}"></span><span>${escapeHtml(entry.label)}</span><code>${escapeHtml(entry.prefix)}</code></li>`
    )
    .join('');
...
<head>
  ...
  <style>
    :root{color-scheme:light;--bg:#f6f7f5;--surface:#fff;--ink:#17201d;--muted:#61706a;--line:#d9dfdb;--accent:#0c7562;--accent-strong:#075b4c;--soft:#e1f1ed;--shadow:0 18px 42px rgba(30,54,46,.09)}
    @media(prefers-color-scheme:dark){:root{color-scheme:dark;--bg:#101715;--surface:#17201d;--ink:#edf4f0;--muted:#aab9b2;--line:#2c3934;--accent:#5bd4bb;--accent-strong:#91e3d0;--soft:#193a33;--shadow:0 18px 42px rgba(0,0,0,.22)}}
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:var(--bg);color:var(--ink);font:16px/1.6 ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}a{color:inherit}button,input{font:inherit}
    .skip{position:absolute;top:-4rem;left:1rem;padding:.5rem;background:var(--ink);color:var(--bg)}.skip:focus{top:1rem}.header{position:sticky;top:0;z-index:2;border-bottom:1px solid var(--line);background:color-mix(in srgb,var(--bg) 90%,transparent);backdrop-filter:blur(14px)}.nav,.shell{width:min(1160px,calc(100% - 40px));margin:auto}.nav{min-height:68px;display:flex;align-items:center;justify-content:space-between;gap:1rem}.brand{display:inline-flex;align-items:center;gap:.65rem;text-decoration:none;font-weight:700}.mark{display:grid;place-items:center;width:32px;height:32px;border:1px solid var(--accent);color:var(--accent);font-weight:800}.links{display:flex;align-items:center;gap:.9rem;color:var(--muted);font-size:.9rem}.links a{text-decoration:none}.links a:hover{color:var(--ink)}.lang,.secondary{border:1px solid var(--line);border-radius:3px;background:var(--surface);color:var(--ink);padding:.42rem .68rem;cursor:pointer}.lang:hover,.secondary:hover{border-color:var(--accent);color:var(--accent)}
