...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{overflow-x:hidden;scroll-behavior:smooth}body{margin:0;overflow-x:hidden;background:var(--bg);color:var(--ink);font:16px/1.6 ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}body.modal-open{overflow:hidden}a{color:inherit}button,input{font:inherit}
...
<body>
  ...
  <script nonce="${escapeHtml(nonce)}">
    (() => {
      ...
      const text = { zh: { astrbot:'AstrBot 官网', docs:'文档', source:'源码', issue:'反馈问题', title:'聚合加速站', copy:'为 <a href="https://astrbot.app/" target="_blank" rel="noreferrer">AstrBot</a> 社区提供的加速站点。加速访问和下载 GitHub、npm、PyPI、Docker Hub 等 70+ 平台的文件，以及代理请求 AI 接口。', note:'面向 AstrBot 开发者，提供稳定的资源入口。', tool:'生成加速地址', hint:'粘贴完整 URL，或输入不带协议的地址。', placeholder:'github.com/user/repository', ready:'加速地址', copyButton:'复制地址', open:'打开地址', copied:'已复制到剪贴板', invalid:'请输入有效的 HTTP(S) 地址。', unsupported:'暂不支持这个域名。请查看文档中的平台清单。', failed:'复制失败，请手动选择地址。', all:'查看全部平台', close:'关闭' }, en: { astrbot:'AstrBot official site', docs:'Docs', source:'Source', issue:'Report issue', title:'Aggregated Accelerator', copy:'An acceleration site for the <a href="https://astrbot.app/" target="_blank" rel="noreferrer">AstrBot</a> community. Accelerate access to GitHub, npm, PyPI, Docker Hub, 70+ platforms, and AI APIs.', note:'Built for AstrBot developers who need dependable resource access.', tool:'Generate an acceleration link', hint:'Paste a full URL, or enter a host without the scheme.', placeholder:'github.com/user/repository', ready:'Acceleration link', copyButton:'Copy link', open:'Open link', copied:'Copied to clipboard', invalid:'Enter a valid HTTP(S) URL.', unsupported:'That host is not supported yet. Check the platform list in the docs.', failed:'Copy failed. Select the URL manually.', all:'View all platforms', close:'Close' } };
      function applyLanguage(){const value=text[lang()];document.documentElement.lang=lang()==='zh'?'zh-CN':'en';for(const node of document.querySelectorAll('[data-i18n]'))node.textContent=value[node.dataset.i18n];for(const [id,key] of [['title','title'],['note','note'],['toolTitle','tool'],['hint','hint'],['resultLabel','ready'],['copy','copyButton'],['open','open'],['moreText','all']])document.getElementById(id).textContent=value[key];document.getElementById('copy').innerHTML=value.copy;input.placeholder=value.placeholder;document.getElementById('lang').textContent=lang()==='zh'?'EN':'中文';document.getElementById('close').setAttribute('aria-label',value.close)}
