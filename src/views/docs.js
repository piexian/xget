...
function catalogSection(category, entries, origin) {
  const rows = entries
    ...
    .map(
      entry =>
        `<tr class="catalog-row" data-search="${escapeHtml(`${entry.label} ${entry.key} ${entry.host} ${entry.prefix}`.toLowerCase())}"><td><div class="catalog-platform"><span class="catalog-icon">${getPlatformIcon(entry.key)}</span><span><strong>${escapeHtml(entry.label)}</strong><small>${escapeHtml(entry.key)}</small></span></div></td><td><code>${escapeHtml(entry.prefix)}</code></td><td><code>https://${escapeHtml(entry.host)}${escapeHtml(entry.basePath)}</code></td><td><code>${escapeHtml(origin)}${escapeHtml(entry.prefix)}</code></td></tr>`
    )
    .join('');
...
<head>
  ...
  <style>
    :root{color-scheme:light;--bg:#fff;--surface:#fff;--ink:#0f172a;--muted:#64748b;--line:#e2e8f0;--accent:#3b82f6;--accent-strong:#2563eb;--soft:#f1f5f9;--code:#f1f5f9;--code-ink:#0f172a}
    @media(prefers-color-scheme:dark){:root{color-scheme:dark;--bg:#0f172a;--surface:#1e293b;--ink:#f8fafc;--muted:#94a3b8;--line:#334155;--accent:#60a5fa;--accent-strong:#3b82f6;--soft:#1e293b;--code:#1e293b;--code-ink:#f8fafc}}
    *{box-sizing:border-box}html{scroll-behavior:smooth;scroll-padding-top:90px}body{margin:0;background:var(--bg);color:var(--ink);font:15px/1.65 ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}a{color:inherit}button,input,select{font:inherit}.shell{width:min(1200px,calc(100% - 40px));margin:auto}.skip{position:absolute;top:-4rem;left:1rem;padding:.5rem;background:var(--ink);color:var(--bg);z-index:5}.skip:focus{top:1rem}.header{position:sticky;top:0;z-index:4;border-bottom:1px solid var(--line);background:color-mix(in srgb,var(--bg) 92%,transparent);backdrop-filter:blur(14px)}.nav{min-height:68px;display:flex;align-items:center;justify-content:space-between;gap:1rem}.brand{display:inline-flex;align-items:center;gap:.65rem;text-decoration:none;font-weight:750}.mark{display:grid;place-items:center;width:32px;height:32px;border:0;border-radius:.5rem;background:linear-gradient(135deg,var(--accent),#60a5fa);color:#fff;font-weight:850}.links{display:flex;align-items:center;gap:1rem;color:var(--muted);font-size:.9rem}.links a{text-decoration:none}.links a:hover{color:var(--ink)}.lang,.copy,.filter,.search{border:1px solid var(--line);border-radius:.5rem;background:var(--soft);color:var(--ink)}.lang,.copy{cursor:pointer}.lang{padding:.42rem .68rem}.lang:hover,.copy:hover{border-color:var(--accent);color:var(--accent)}
    .intro{padding:5.4rem 0 3.8rem;border-bottom:1px solid var(--line)}.eyebrow{margin:0 0 1rem;color:var(--accent);font-size:.74rem;font-weight:750;letter-spacing:.14em;text-transform:uppercase}.intro h1{max-width:900px;margin:0;font-size:clamp(2.6rem,6vw,5rem);line-height:1;letter-spacing:-.045em;text-wrap:balance}.intro-copy{max-width:720px;margin:1.35rem 0 0;color:var(--muted);font-size:1.08rem}.stats{display:flex;gap:2rem;margin-top:2rem}.stat strong{display:block;font-size:1.5rem}.stat span{color:var(--muted);font-size:.8rem}.layout{display:grid;grid-template-columns:210px minmax(0,1fr);gap:clamp(2rem,6vw,5rem);align-items:start}.toc{position:sticky;top:92px;padding:2.4rem 0}.toc strong{display:block;margin-bottom:.8rem;font-size:.75rem;color:var(--muted);letter-spacing:.12em;text-transform:uppercase}.toc a{display:block;padding:.35rem 0;color:var(--muted);text-decoration:none}.toc a:hover{color:var(--accent)}.content{min-width:0}.doc-section{padding:3.8rem 0;border-bottom:1px solid var(--line)}.section-heading{display:grid;grid-template-columns:minmax(0,.7fr) minmax(260px,1fr);gap:2rem;margin-bottom:2rem}.section-heading h2{margin:0;font-size:clamp(1.6rem,3vw,2.25rem);letter-spacing:-.03em}.section-heading p{margin:.25rem 0 0;color:var(--muted)}.anatomy{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));border:1px solid var(--line);border-radius:.5rem;overflow:hidden}.anatomy div{padding:1.1rem;border-right:1px solid var(--line);background:var(--surface)}.anatomy div:last-child{border:0}.anatomy small{display:block;color:var(--muted)}.anatomy code{display:block;margin-top:.35rem;color:var(--accent);overflow-wrap:anywhere}.rules{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1px;background:var(--line);border:1px solid var(--line);border-radius:.5rem;overflow:hidden}.rule{min-width:0;background:var(--bg);padding:1rem}.rule strong{display:block;margin-bottom:.45rem}.rule code{display:block;color:var(--muted);font-size:.78rem;overflow-wrap:anywhere}.rule .arrow{color:var(--accent);padding:.2rem 0}.commands{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem}.command{min-width:0;border:1px solid var(--line);border-radius:.5rem;background:var(--surface);box-shadow:0 1px 3px rgba(0,0,0,.1)}.command-head{height:42px;display:flex;align-items:center;justify-content:space-between;padding:0 .7rem 0 1rem;border-bottom:1px solid var(--line);font-size:.8rem;color:var(--muted)}.copy{padding:.28rem .58rem;font-size:.75rem}.command pre{min-height:126px;margin:0;padding:1rem;overflow:auto;background:var(--code);color:var(--code-ink);font:12.5px/1.65 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}.command code{font:inherit}.callout{margin-top:1rem;padding:1rem 1.1rem;border-left:3px solid var(--accent);background:var(--soft);color:var(--muted)}
    .catalog-tools{display:grid;grid-template-columns:1fr 190px;gap:.7rem;margin-bottom:1.5rem}.search,.filter{width:100%;padding:.7rem .8rem}.search:focus,.filter:focus{outline:3px solid color-mix(in srgb,var(--accent) 28%,transparent);border-color:var(--accent)}.catalog-group{margin-top:2.4rem}.catalog-group[hidden]{display:none}.group-title{display:flex;align-items:end;justify-content:space-between;margin-bottom:.7rem}.group-title h3{margin:0;font-size:1.2rem}.group-title h3 span{margin-left:.65rem;color:var(--muted);font-size:.76rem;font-weight:500}.count{color:var(--muted);font-variant-numeric:tabular-nums}.table-wrap{overflow-x:auto;border-top:1px solid var(--ink)}table{width:100%;border-collapse:collapse;min-width:760px;text-align:left}th,td{padding:.75rem .7rem;border-bottom:1px solid var(--line);vertical-align:top}th{color:var(--muted);font-size:.72rem;text-transform:uppercase}td{font-size:.83rem}td strong,td small{display:block}td small{color:var(--muted)}td code{font:12px/1.5 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;overflow-wrap:anywhere}.catalog-platform{display:flex;align-items:center;gap:.65rem}.catalog-icon{display:grid;place-items:center;width:22px;height:22px;flex:0 0 22px}.catalog-icon svg{display:block;width:19px;height:19px}.empty{display:none;padding:2rem;border:1px solid var(--line);color:var(--muted);text-align:center}.footer{padding:1.4rem 0 2.5rem;color:var(--muted);font-size:.85rem}.footer-inner{display:flex;justify-content:space-between;gap:1rem;flex-wrap:wrap}.footer-links{display:flex;gap:1rem}:focus-visible{outline:3px solid color-mix(in srgb,var(--accent) 45%,transparent);outline-offset:2px}
...
<body>
  ...
  <header class="header"><nav class="shell nav" aria-label="Primary navigation"><a class="brand" href="/"><span class="mark" aria-hidden="true">🌟</span><span>聚合加速站</span></a><div class="links"><a href="/" data-zh="首页" data-en="Home">首页</a><a class="source" href="https://github.com/xixu-me/Xget" target="_blank" rel="noreferrer" data-zh="源码" data-en="Source">源码</a><button class="lang" id="lang" type="button">EN</button></div></nav></header>
